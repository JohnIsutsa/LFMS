		...
		if (isset($_POST['uploadphoto'])) {
			...
			elseif ($_FILES['image']['size'] <= 0 || $_FILES['image']['size'] > 1024000 ) {
					echo "<script>alert('Image size is not proper');
 						window.location.href='lawyeruserprofile.php';</script>";
 			}
 			elseif (!in_array($ext, $validExt)){
        		...
        		window.location.href='lawyeruserprofile.php';</script>";
			        ...
			        $queryupdate = "UPDATE lawyer SET image = '$picture' WHERE lawyer_id= '$userid' " ;
				...
				<?php
					...
					$name = $validated_data['name'];
     				$useremail = $validated_data['email'];
      				...
      				$updatequery1 = "UPDATE lawyer SET lawyer_name = '$name' , email='$useremail' , bio='$userbio' WHERE lawyer_id = '$userid' " ;
      				$result2 = mysqli_query($conn , $updatequery1) or die(mysqli_error($conn));
					if (mysqli_affected_rows($conn) > 0) {
						echo "<script>alert('PROFILE UPDATED SUCCESSFULLY');
    					window.location.href='lawyerviewprofile.php?name=$username'</script>";
					} else {
						...
						echo  "<center><font color='red'>New password and Confirm New password do not match </font></center>";
						  ...
						  $updatequery = "UPDATE lawyer SET password = '$userpassword', lawyer_name='$name', email= '$useremail' WHERE lawyer_id='$userid'";
