		...
		if (isset($_POST['uploadphoto'])) {
			...
			elseif ($_FILES['image']['size'] <= 0 || $_FILES['image']['size'] > 1024000 ) {
					echo "<script>alert('Image size is not proper');
 						window.location.href='clientuserprofile.php';</script>";
 			}
 			elseif (!in_array($ext, $validExt)){
        		...
        		window.location.href='clientuserprofile.php';</script>";
			        ...
			        $queryupdate = "UPDATE clients SET image = '$picture' WHERE client_id= '$userid' " ;
				...
				<?php
					...
					$name = $validated_data['name'];
     				$useremail = $validated_data['email'];
      				...
      				$updatequery1 = "UPDATE clients SET client_name = '$name' , email='$useremail' , bio='$userbio' WHERE client_id = '$userid' " ;
      				$result2 = mysqli_query($conn , $updatequery1) or die(mysqli_error($conn));
					if (mysqli_affected_rows($conn) > 0) {
						echo "<script>alert('PROFILE UPDATED SUCCESSFULLY yaass');
    					window.location.href='clientviewprofile.php?name=$username'</script>";
					} else {
						...
						echo  "<center><font color='red'>New password and Confirm New password do not match </font></center>";
						  ...
						  $updatequery = "UPDATE clients SET password = '$userpassword', client_name='$name', email= '$useremail' WHERE client_id='$userid'";
